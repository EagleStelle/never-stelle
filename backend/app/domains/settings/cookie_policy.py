...
def cookie_policy_defaults() -> dict[str, Any]:
    return {
...
def _resolved_policies() -> dict[str, CookiePolicy]:
    ...
    with _cache_lock:
        if _policies_cache is None:
            _policies_cache = {
                key: CookiePolicy(**policy) for key, policy in get_effective_cookie_policies().items()
            }
        return _policies_cache


def cookie_policy_for_source(source_key: Any) -> CookiePolicy:
    return _resolved_policies().get(normalize_source_key(source_key), DEFAULT_COOKIE_POLICY)
