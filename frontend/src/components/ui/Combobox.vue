...
<template>
  <ComboboxRoot
    ...
    @update:model-value="(val) => { if (val) emit('update:modelValue', (val as any).key) }"
    v-model:searchTerm="searchTerm"
    @update:open="handleOpenChange"
    :display-value="(val) => (val as any)?.label || ''"
  >
    <ComboboxAnchor :class="['relative inline-flex items-center rounded-lg border border-primary-muted bg-secondary-muted focus-within:border-primary focus-within:ring-2 focus-within:ring-primary h-10 transition-all duration-200', props.class]">
    ...
    <ComboboxPortal>
      <ComboboxContent 
        class="z-50 min-w-[160px] bg-secondary border border-primary-muted rounded-lg shadow-lg overflow-hidden" 
        position="popper" 
        :side-offset="4"
      >
        <ComboboxViewport class="p-1">
          ...
          <ComboboxItem 
            ...
            :key="item.key" 
            :value="item"
            class="relative flex w-full cursor-pointer select-none items-center rounded-md py-2 pl-8 pr-2 text-sm outline-none focus:bg-primary focus:text-background text-text-muted data-[highlighted]:bg-primary data-[highlighted]:text-background transition-colors"
          >
