...
function getStatusBadgeClass(status: string): string {
  ...
  if (status === "running") return "border-primary bg-primary/20 text-text";
  if (status === "completed") return "border-primary-muted bg-primary-subtle/30 text-text";
  if (status === "failed") return "border-red-600 bg-red-600 text-white dark:border-red-500 dark:bg-red-500";
  return "border-primary-muted bg-secondary-muted text-text-muted";
}

function getSiteBadgeClass(siteCategory: string): string {
  if (siteCategory === "youtube") return "border-primary bg-primary/20 text-text";
  if (siteCategory === "facebook") return "border-primary-muted bg-primary-subtle/30 text-text";
  if (siteCategory === "instagram") return "border-primary bg-primary/20 text-text";
  if (siteCategory === "tiktok") return "border-primary-muted bg-primary-subtle/30 text-text";
  return "border-primary-muted bg-secondary-muted text-text-muted";
}
</script>
...
<template>
  <section class="grid grid-cols-1 gap-2" :class="{ 'block': viewMode === 'table' }" aria-live="polite">
    <div v-if="props.loading" class="border border-dashed border-primary-muted rounded-lg bg-secondary flex min-h-32 items-center justify-center gap-2 text-text-muted text-center">
      <IconSpinner class="animate-spin" aria-hidden="true" />
      <span>Loading downloads...</span>
    </div>

    <div v-else-if="props.errorMessage" class="border border-dashed border-primary-muted rounded-lg bg-secondary flex min-h-32 items-center justify-center gap-2 text-text-muted text-center border-primary text-text">
      {{ props.errorMessage }}
    </div>

    <div v-else-if="viewMode === 'table'" class="w-full overflow-auto rounded-lg">
      <table class="w-full min-w-full border-separate border-spacing-0 overflow-hidden text-left border border-primary-muted rounded-lg bg-secondary">
        <thead>
          <tr>
            <th>Site</th>
            <th>Source</th>
        ...
        <tbody>
          <tr v-for="task in props.tasks" :key="task.vid">
            <td>
              <span class="inline-flex max-w-full items-center border rounded-full px-2 py-1 tracking-wide leading-none uppercase" :class="getSiteBadgeClass(task.site_category)">
                {{ task.site_label }}
              </span>
            </td>
            <td>
            ...
            <td>
              <span class="inline-flex max-w-full items-center border rounded-full px-2 py-1 tracking-wide leading-none uppercase" :class="getStatusBadgeClass(task.status)">
                {{ task.status_label }}
              </span>
            </td>
            ...
            <td>
              <div class="flex items-center gap-1.5">
                <button
                  ...
                  class="inline-flex items-center justify-center gap-1.5 min-h-8 border border-primary-muted rounded-lg bg-secondary-muted text-text-muted leading-none transition-all duration-200 ease-out active:scale-95 h-8 px-2.5 hover:border-primary hover:text-text"
                  type="button"
                ...
                <button
                  ...
                  class="inline-flex items-center justify-center gap-1.5 min-h-8 border border-primary-muted rounded-lg bg-secondary-muted text-text-muted leading-none transition-all duration-200 ease-out active:scale-95 w-8 h-8 hover:border-primary hover:text-text border-text bg-primary text-background hover:border-primary hover:bg-text hover:text-background"
                  type="button"
    ...
    <template v-else>
      <article v-for="task in props.tasks" :key="task.vid" class="border border-primary-muted rounded-lg bg-secondary p-4">
        <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-4">
          ...
          <div class="flex items-start justify-end gap-1.5">
            <button
              ...
              class="inline-flex items-center justify-center gap-1.5 h-8 w-8 border border-primary-muted rounded-lg bg-secondary-muted text-text-muted leading-none transition-all duration-200 ease-out active:scale-95 hover:border-primary hover:text-text"
              type="button"
              aria-label="Download file"
              title="Download file"
              @click="emit('download', task.vid)"
            ...
            <button
              ...
              class="inline-flex items-center justify-center gap-1.5 h-8 px-3 border border-primary-muted rounded-lg bg-secondary-muted text-text-muted leading-none transition-all duration-200 ease-out active:scale-95 hover:border-primary hover:text-text"
              type="button"
              aria-label="Delete from history"
              title="Delete from history"
              @click="emit('hide', task.vid)"
