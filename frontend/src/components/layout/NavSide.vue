...
<template>
  <aside 
    class="hidden lg:flex sticky top-0 h-dvh min-w-0 flex-col gap-2 bg-secondary py-3 px-2.5 border-r border-secondary-muted" 
  ...
  >
    ...
    <nav class="flex gap-1" :class="isExpanded ? 'w-full flex-col' : 'flex-col items-center w-full'" aria-label="App navigation">
      <button
        ...
        :key="item.key"
        type="button"
        class="inline-flex items-center rounded-lg bg-transparent text-text-muted leading-none active:scale-95 hover:text-text hover:bg-secondary-muted aria-pressed:bg-primary aria-pressed:text-background"
    ...
    <div class="flex gap-1 mt-auto" :class="isExpanded ? 'w-full flex-col' : 'flex-col items-center w-full'">
      ...
      <button 
        type="button" 
        class="inline-flex items-center rounded-lg bg-transparent text-text-muted leading-none active:scale-95 hover:text-text hover:bg-secondary-muted aria-pressed:bg-primary aria-pressed:text-background"
        :class="[isExpanded ? 'w-full flex-row justify-start px-3.5 h-10 gap-3' : 'w-10 h-10 justify-center', activePage === 'settings' ? 'bg-primary text-background' : '']"
