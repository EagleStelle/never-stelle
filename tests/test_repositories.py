...
def test_learned_formats_persist_to_learned_formats_table(tmp_path, monkeypatch):
    ...
    with database_module.transaction() as connection:
        rows = connection.execute(
            "SELECT source_key, templates FROM learned_formats"
        ).fetchall()
...
def test_enrichment_jobs_claim_retry_and_complete_persistently(tmp_path, monkeypatch):
    ...
    assert repositories.count_enrichment_jobs_payload(("pending",)) == 1
    ...
    assert repositories.count_enrichment_jobs_payload(("pending",)) == 0
